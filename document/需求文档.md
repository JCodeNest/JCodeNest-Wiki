...
系统采用 SpringCloud 微服务架构，按业务领域和功能职责划分为10个核心服务：

**核心服务说明：**
- **网关服务（service-gateway）**：统一入口路由、认证授权、请求限流、日志记录
- **通用服务（service-common）**：公共工具类、统一响应格式、异常处理、常量定义
- **用户服务（service-user）**：用户注册、登录、权限管理、用户画像
- **内容服务（service-content）**：文档管理、版本控制、分类标签、内容审核
- **支付服务（service-payment）**：订单管理、支付集成、优惠券、收益分成
- **学习服务（service-learning）**：课程管理、学习进度、考试系统、数据分析
- **搜索服务（service-search）**：全文检索、智能推荐、内容发现
- **通知服务（service-notification）**：消息推送、邮件通知、站内信
- **文件服务（service-file）**：文件上传、存储管理、CDN加速
- **AI服务（service-ai）**：智能问答、内容推荐、自动标签
...
graph TB
    ...
    subgraph "网关层"
        GatewayService[网关服务<br/>service-gateway]
    end

    subgraph "通用层"
        CommonService[通用服务<br/>service-common]
    end

    subgraph "微服务层"
        UserService[用户服务<br/>service-user]
        ContentService[内容服务<br/>service-content]
        PaymentService[支付服务<br/>service-payment]
        LearningService[学习服务<br/>service-learning]
        SearchService[搜索服务<br/>service-search]
        NotificationService[通知服务<br/>service-notification]
        FileService[文件服务<br/>service-file]
        AIService[AI服务<br/>service-ai]
    end
    ...
    GatewayService --> ContentService
    GatewayService --> PaymentService
    GatewayService --> LearningService
    GatewayService --> SearchService
    GatewayService --> NotificationService
    GatewayService --> FileService
    GatewayService --> AIService

    UserService --> CommonService
    ContentService --> CommonService
    PaymentService --> CommonService
    LearningService --> CommonService
    SearchService --> CommonService
    NotificationService --> CommonService
    FileService --> CommonService
    AIService --> CommonService

    GatewayService --> Nacos
    CommonService --> Nacos
    UserService --> Nacos
    ...
    style GatewayService fill:#ff9999
    style CommonService fill:#99ccff
```
...
graph TB
    ...
    subgraph "微服务集群"
        ...
        subgraph "内容服务集群"
            CS1[service-content-1]
            CS2[service-content-2]
        end

        subgraph "支付服务集群"
            PS1[service-payment-1]
            PS2[service-payment-2]
        end
...
- 搭建Spring Cloud微服务架构
- 配置Nacos注册中心和配置中心
- 开发service-gateway网关服务
- 开发service-common通用服务
